    ...
    useEffect(() => {
        alanBtn({
            ...
            onCommand: (commandData) => {
                if (commandData.command === 'gethealth') {
                   setHealth(commandData.data)
                   console.log(health)
                }
                if (commandData.command === 'gettips') {
  ...
  <div id={"initiatives"}>
      <section className="text-black mt-12">
        <div class="max-w-screen-xl px-4 py-15 mx-auto sm:px-6 lg:px-8">
          ...
          {health.map((h) =>    <div className={" block p-8 transition shadow-xl rounded-xl hover:shadow-pink-500/10 hover:border-pink-500/10"}>
          <h3 className="mt-4 text-xl font-bold text-center text-indblue">{h.name}</h3>
